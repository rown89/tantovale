...
export const getFiltersBySubcategoryFiltersIdService = async (c: Context<AppBindings>, id: number) => {
	const { db } = createClient();
	...
	type FilterWithValues = {
		...
		options: Array<{
			...
			value: FilterRow['fv_value'] | FilterRow['fv_number_value'] | FilterRow['fv_boolean_value'];
		}>;
	};
