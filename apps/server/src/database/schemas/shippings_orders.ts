...
export const shippings_orders = pgTable(
	'shippings_orders',
	{
	...
	(table) => [
		foreignKey({
			columns: [table.shipping_id],
			foreignColumns: [shippings.id],
			name: 'shippings_orders_shipping_id_fkey',
		}),
		foreignKey({
			columns: [table.order_id],
			foreignColumns: [orders.id],
			name: 'shippings_orders_order_id_fkey',
		}),
	],
);
