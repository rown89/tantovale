...
import { createItemSchema, multipleImagesSchema } from '@workspace/server/extended_schemas';
...
export function isNextButtonEnabled({
	...
	const requiredProperties =
		subCatProperties?.filter((item) => item.slug !== 'delivery_method' && item.on_item_create_required) || [];
	...
	const isMandatoryPropertiesSatisfied = satisfiedProperties === requiredProperties.length;
...
export function updatePropertiesArray({
	...
	const currentProperties: Omit<PropertyFormValue, 'name'>[] = Array.isArray(field.state.value)
		? [...field.state.value]
		: [];
	...
	const existingIndex = currentProperties.findIndex((prop) => String(prop.id) === String(property.id));
...
export function handleItemPreviewProperties({
	...
	const conditionProperty = properties.find((item) => item.slug === 'condition');
	const selectedConditionId = conditionProperty?.value;

	let selectedCondition;

	if (selectedConditionId && subCatProperties) {
		const conditionFilter = subCatProperties.find((item) => item.slug === 'condition');

		const extractedCondition = conditionFilter?.options.find((item) => item.id === Number(selectedConditionId));
	...
	const deliveryProperty = properties.find((item) => item.slug === 'delivery_method');
	...
	if (selectedDeliveryMethodsIds && subCatProperties) {
		const deliveryMethodFilter = subCatProperties.find((item) => item.slug === 'delivery_method');
