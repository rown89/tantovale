	...
	return (
		<div className='space-y-2'>
			...
			{property.type === 'select' && (
				<>
					<Label htmlFor={field.name} className='block text-slate-500 dark:text-slate-400'>
						{property.name} {property.on_item_create_required && <span className='text-red-500'>*</span>}
					</Label>
					<Select
						name={field.name}
						onValueChange={(value) => {
						...
						<SelectContent>
							<SelectGroup>
								{!property.on_item_create_required && <SelectItem value='reset'>--</SelectItem>}
					...
					{property.on_item_create_required && <FieldInfo field={field} propertyName={property.name} />}
				</>
			)}
			...
			{property.type === 'boolean' && (
				<div className='flex flex-col gap-2'>
					...
					<Switch
						checked={getCurrentValue(field, property.id) !== undefined ? getCurrentValue(field, property.id) : false}
			...
			{property.type === 'number' && (
				<>
					...
					<Input
						...
						id={field.name}
						value={
							getCurrentValue(field, property.id) !== undefined ? getCurrentValue(field, property.id).toString() : ''
						}
						onChange={(e) => {
			...
			{property.type === 'checkbox' && (
				<div className='flex flex-col gap-2'>
					...
					<div className={`flex gap-4 ${property.options.length > 3 ? 'flex-col' : 'flex-row'}`}>
							...
							<div key={item.id} className='flex items-center gap-2'>
								<Checkbox
									...
									checked={
										...
										getCurrentValue(field, property.id)?.includes(item.id.toString())
									}
									onCheckedChange={(checked) => {
										const currentValues = Array.isArray(getCurrentValue(field, property.id))
										...
										if (checked) {
											...
											updatePropertiesArray({
												value: currentValues.filter((id) => id !== item.id.toString()),
												property,
												field,
											});
										}
			...
			{property.type === 'radio' && (
				<div className='flex flex-col gap-4'>
					...
					<RadioGroup
						...
						className={`flex gap-4 ${property.options.length > 3 ? 'flex-col' : 'flex-row'}`}>
						{property.options.map((item) => (
							<div key={item.id} className='flex items-center gap-2'>
								<RadioGroupItem id={`${field.name}-${item.id}`} value={item.id?.toString()} />
								<Label htmlFor={`${field.name}-${item.id}`}>{item.name}</Label>
							</div>
						))}
					</RadioGroup>
