...
const useSmoothLogoMovement = (mousePosition: React.RefObject<{ x: number; y: number }>) => {
	...
	useEffect(() => {
		...
		const updateLogoPosition = () => {
			...
			setLogoPosition((current) => {
				...
				const newRotateX = current.rotateX + (targetRotateX - current.rotateX) * 0.05;
				const newRotateY = current.rotateY + (targetRotateY - current.rotateY) * 0.05;
...
// Typewriter effect
const useTypewriter = (text: string, speed: number = 100, delay: number = 100) => {
...
const useCanvasAnimation = (mousePosition: React.MutableRefObject<{ x: number; y: number }>) => {
...
export default function Home() {
	...
	return (
		<div
			...
			style={{
				background: 'radial-gradient(circle at center, #333333 0%, #111111 50%, #000000 100%)',
			}}>
			...
			<div className='relative z-10 flex min-h-screen flex-col items-center justify-center px-4 text-white'>
				<div className='mx-auto flex max-w-3xl flex-col items-center text-center'>
					<div
						className={`relative mx-auto mb-12 h-48 w-48 transition-opacity duration-1000 ease-in-out ${logoVisible ? 'opacity-100' : 'opacity-0'}`}
						style={{
					...
					<h1 className='mb-8 inline-flex flex-wrap items-center justify-center gap-2 bg-gradient-to-r from-blue-200 via-purple-200 to-pink-200 bg-clip-text text-3xl font-bold leading-tight text-transparent md:gap-2 md:text-4xl lg:text-5xl'>
						{text1}
						{showCaret1 && !isComplete1 && (
							<span className='ml-1 animate-pulse text-3xl text-white opacity-60 md:text-4xl lg:text-5xl'>|</span>
						)}
						{isComplete1 && (
							<>
								<span className='flex bg-gradient-to-r from-yellow-200 to-yellow-400 bg-clip-text text-3xl text-transparent md:text-4xl lg:text-5xl'>
									{' ' + text2}
									<div className='flex w-2 items-center justify-center'>
										{showCaret2 && (
											<span className='ml-1 animate-pulse text-3xl text-white opacity-60 md:text-4xl lg:text-5xl'>
												|
											</span>
										)}
									</div>
					...
					<div
						className={`mb-12 flex items-center justify-center transition-opacity duration-1000 ease-in-out ${githubVisible ? 'opacity-100' : 'opacity-0'}`}>
						<Link
							href='https://github.com/rown89/tantovale'
							target='_blank'
							rel='noopener noreferrer'
							className='inline-flex items-center space-x-2 rounded-md bg-white/10 px-4 py-2 text-sm font-medium transition-colors hover:bg-white/20'>
					...
					<div
						className={`flex flex-col items-center gap-2 transition-opacity duration-1000 ease-in-out md:flex-row ${githubVisible ? 'opacity-100' : 'opacity-0'}`}>
						<h2 className='text-xl font-medium'>Stato:</h2>
						<div className='inline-block rounded-full bg-blue-500/30 px-3 py-1 text-sm font-medium'>In costruzione</div>
					</div>
				</div>
			</div>
		</div>
