...
export async function fetchItemDetailData({
  id,
  authTokens,
}: ItemDetailPageParams): Promise<ItemDetailData> {
  ...
  const [itemOwnerData, userData, chatId, orderProposal, isFavorite] =
    await Promise.allSettled([
      ...
      authHeaders
        ? fetchOrderProposalData(id, authHeaders)
        : Promise.resolve(undefined),
      authHeaders
        ? fetchFavoriteStatus(id, authHeaders)
  ...
  const resolvedOrderProposal =
    orderProposal.status === fulfilled ? orderProposal.value : undefined;
  const resolvedIsFavorite =
    isFavorite.status === fulfilled ? isFavorite.value : false;
