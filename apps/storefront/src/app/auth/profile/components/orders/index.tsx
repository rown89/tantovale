...
export default function UserSellingItemsComponent() {
  ...
  const { data: orders = [], isLoading: isOrdersLoading } = useQuery({
    ...
    queryFn: async () => {
      const userOrderListResponse = await client.orders.auth.status[
        ":status"
      ].$get({
        param: {
          status: statusFilter,
  ...
  return (
    <div className="flex flex-col w-full overflow-auto px-4 gap-7">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="payment_refunded">
                        Payment Refunded
                      </SelectItem>
                      <SelectItem value="shipping_pending">
                        Shipping Pending
                      </SelectItem>
                      <SelectItem value="shipping_confirmed">
                        Shipping Confirmed
                      </SelectItem>
                      ...
                      {orders.length > 0 ? (
                        orders.map((order) => (
                          <TableRow key={order.id}>
                            ...
                            <TableCell className="truncate max-w-[280px] font-bold">
                              <Link
                                href={`/item/${linkBuilder({
                                  id: order.item.id,
                                  title: order.item.title,
                                })}`}
                                className="hover:underline hover:text-accent text-primary"
                              >
                                {order.item.title}
                              </Link>
                            </TableCell>
                            ...
                            <TableCell>
                              € {formatPrice(order.finished_price)}
                            </TableCell>
                            <TableCell>
