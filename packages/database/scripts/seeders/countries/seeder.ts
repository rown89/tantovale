...
import { countries, states, cities, regions, subRegions } from '@workspace/database/schemas/schema';
...
async function batchInsert<T extends Record<string, any>>(
	...
	for (let i = 0; i < data.length; i += BATCH_SIZE) {
		...
		console.log(`⏳ Progress: ${i + batch.length}/${data.length} processed, ${inserted} inserted`);
	}

	console.log(`✅ Completed ${entityName} seeding: ${inserted} records inserted`);
...
async function insertCountries(): Promise<number> {
	const countriesData = readJsonFile<typeof countries & { subregion_id: number | null; region_id: number | null }>(
		'./countries.json',
	);
...
async function insertStates(): Promise<number> {
	...
	return await batchInsert<Omit<typeof states, 'id'>>(states, statesReshape, 'states');
}
...
async function insertCities(): Promise<number> {
	...
	return await batchInsert<Omit<typeof cities, 'id'>>(cities, citiesReshape, 'cities');
}
